...
if [ "$IS_DRAFT" = true ] ; then
    ...
    bundle exec fastlane upload_to_play_store --aab /builds/lidholm/listanything/app-production-release.aab --track alpha   --release_status draft
else
    echo "Deploying non draft app"
    bundle exec fastlane upload_to_play_store --aab /builds/lidholm/listanything/app-production-release.aab --track alpha
fi
