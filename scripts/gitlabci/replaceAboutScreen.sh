echo "Replacing version"
 ...
 sed -i'' -e "s/0.0.1+1/$VERSION_COUNT /g" lib/standard/version/version.dart
 cat pubspec.yaml
 echo "===---===="

 GIT_HASH=`git rev-parse --short HEAD`
 echo $GIT_HASH
 sed -i'' -e "s/<git_hash>/$GIT_HASH/g" lib/custom/pages/about/about_page.dart

 VERSION=`grep "version:" pubspec.yaml | cut -d ' ' -f 2 | xargs echo `
 echo $VERSION
 sed -i'' -e "s/<version>/$VERSION/g" lib/custom/pages/about/about_page.dart

 DATETIME=`date +"%Y-%m-%d %k:%M:%S %Z"`
 echo $DATETIME
 sed -i'' -e "s/<datetime>/$DATETIME/g" lib/custom/pages/about/about_page.dart

 cat lib/custom/pages/about/about_page.dart
